...
local font14 = render.setup_font("C:/Windows/Fonts/lucon.ttf", 12)
local font16 = render.setup_font("C:/Windows/Fonts/lucon.ttf", 13)
local titleFont = render.setup_font("C:/Windows/Fonts/lucon.ttf", 22)
...
local function loadSettings()
    ...
    if menu_wallbang_helper_file.value > #wallbang_helper_files then
        ...
        print("[Femboy.lua] Wallbang helper file index was out of bounds, reset to 1")
    end
...
local function renderGui()
    ...
    if mouse.click and 
       ...
       mouse.pos.y <= icon_y + icon_size then
        saveSettings()
        print("[Femboy.lua] Settings saved!")
    end
...
local state_font = render.setup_font("C:/windows/fonts/lucon.ttf", 18, 600)
...
local font = render.setup_font("C:/windows/fonts/lucon.ttf", 12, 400)
local warningfont = render.setup_font("C:/windows/fonts/lucon.ttf", 27, 400)
...
local verdana = render.setup_font("C:/Windows/Fonts/lucon.ttf", 12, 0)

local cheat_name = "Femboy.lua" 
local custom_username = false
local custom_username_text = "Femboy"
...
local font = render.setup_font("C:/Windows/Fonts/lucon.ttf", 25)
local small_font = render.setup_font("C:/Windows/Fonts/lucon.ttf", 12)
...
local function load_locations()
    ...
    if not file then
        print("[Femboy.lua:WallbangHelper] Failed to load config file (" .. wallbang_helper_files[menu_wallbang_helper_file.value] .. "). Please select a different one from the dropdown!")
        return false
    end
    ...
    if not success or not parsed then
        print("[Femboy.lua:WallbangHelper] Failed to parse config file: " .. (parsed or "unknown error"))
        return false
    end
    ...
    state.loaded = true
    print("[Femboy.lua:WallbangHelper] Loaded " .. #state.locations .. " locations")
    return true
end
...
local function draw_wallbangs()
    ...
    if current_map ~= state.current_map then
        ...
        print("[Femboy.lua:WallbangHelper] Current Map: " .. current_map)
    end
...
print("[Femboy.lua] Script loaded!")
...
register_callback("unload", function()
    ...
    print("[Femboy.lua] Script unloaded!")
end)
